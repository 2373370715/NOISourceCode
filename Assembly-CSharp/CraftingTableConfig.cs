...
public class CraftingTableConfig : IBuildingConfig
{
	...
	private void ConfigureRecipes()
	{
		...
		}))
		{
			...
			DisposableElectrobankConfig.recipes.Add("DisposableElectrobank_UraniumOre".ToTag(), new ComplexRecipe(id, array, array2, new string[]
			{
				"DLC3_ID"
			})
			{
	...
	private void CreateMetalMiniVoltRecipe(Tag inputMetal)
	{
		...
		{
			time = INDUSTRIAL.RECIPES.STANDARD_FABRICATION_TIME * 2f,
			description = string.Format(STRINGS.BUILDINGS.PREFABS.CRAFTINGTABLE.RECIPE_DESCRIPTION, inputMetal.ProperName(), STRINGS.ITEMS.INDUSTRIAL_PRODUCTS.ELECTROBANK_METAL_ORE.NAME),
			nameDisplay = ComplexRecipe.RecipeNameDisplay.ResultWithIngredient,
			fabricators = new List<Tag>
			{
				"CraftingTable"
